...
public class WeaponMenuManager : MonoBehaviour
{
    ...
    public VerticalLayoutGroup inactiveWeaponGrid;
    public int curWeaponPage = 0;
    public int curOHWeaponPage = 0;
    ...
    private void OnEnable()
    {
        ...
        if (PlayerWeaponManager.instance.GetOffHand() != null)
            curOHWeaponPage = PlayerWeaponManager.instance.ownedSpecialWeapons.IndexOf(PlayerWeaponManager.instance.GetOffHand().gameObject);
        //cmpntScroll.value = 0;
        curComponentPage = 0;
        activeWeapon = null;
        ReloadWeaponMenu();
        //// Do this to load inactive weapons
        //LoadInactiveWeapons();
        //input
    ...
    void HandleSwitchWeaponInput()
    {
        ...
        int totalWeapons = mainHandActive? PlayerWeaponManager.instance.ownedWeapons.Count 
            :PlayerWeaponManager.instance.ownedSpecialWeapons.Count;
        ...
        // get reference to the correct page #
        ref int page = ref mainHandActive? ref curWeaponPage :ref curOHWeaponPage;
        if (switchWeaponUp){
            switchWeaponUp = false;
            if (page < totalWeapons - 1){
                page++;
                LoadWeaponsToScreen();
                DisplayActiveWeapon();
    ...
    public string BreakDownActiveWeapon(bool checkOnly=false)
    {
        ...
        catch (Exception e) //Catches not lvl 5 or over error / no active weapon
        {
            if(!checkOnly)
                Debug.Log(e.Message);
            salvageErrorWindow.GetComponent<TooltipUI>().centerText.text = e.Message;
            return (e.Message);
        }
        return ("CanSalvage");
    }
    ...
    public void WeaponScroll(float value)
    {
        ...
        int numOfPage = Mathf.CeilToInt(weaponsCount / (float)wpnPerRow + 1);
        if (numOfPage < 2)
        {
        ...
        if (page > numOfPage){// past the end go to beg
            page = 0;
        }
        else if (page < 0){//past beginning go to end
            page = numOfPage;
        }
    ...
    public void ComponentScroll(float value)
    {
        ...
        int numOfPage = Mathf.CeilToInt(count / (float)cmpntPerRow);
        if (numOfPage < 2)
        {
    ...
    void LoadWeaponEvolveButtons()
    {
        ...
        if (evolves.Count >= 1 && evolves[0].Trim().Length > 0) {
            ...
            wpnEvolveBtn1.SetActive(false);
        if (evolves.Count >= 2 && evolves[1].Trim().Length > 0) {//2nd weapon evolve
    ...
    void LoadWeaponsToScreen(bool loadInactive = false)
    {
        PlayerWeaponManager playerWpns = PlayerWeaponManager.instance;

        List<GameObject> weapons = new List<GameObject>();
        bool loadMainHands = mainHandActive;
        if(loadInactive)
            loadMainHands = !loadMainHands;
        weapons.AddRange(loadMainHands ? playerWpns.ownedWeapons : playerWpns.ownedSpecialWeapons);
        //weapons.AddRange(playerWpns.ownedSpecialWeapons);

        int maxDisplayed = 3;

        Transform gridTransform = loadInactive ? inactiveWeaponGrid.transform : weaponsGrid.transform;
        foreach (Transform child in gridTransform)
            Destroy(child.gameObject);

        displayed = 0;
        int page = (loadMainHands ? curWeaponPage : curOHWeaponPage);
        //Debug.Log("mainHandActive="+ mainHandActive + ",Page=" + page);
        for (int i = page; i < weapons.Count && displayed < maxDisplayed; i++){
            ...
            GameObject gridElement = Instantiate(this.weaponButton, gridTransform);
            WeaponButtonUI weaponButton = gridElement.GetComponent<WeaponButtonUI>();

            if (!loadInactive && weaponButton.tooltip != null)
            ...
            if (!loadInactive) {
                // First weapon displayed becomes active weapon
                if (displayed == 1){
                    activeWeapon = wpnScrpt.gameObject;
                }
                // mark equipped weapon
                if ((mainHandActive && i == playerWpns.indexOfEquippedWeapon) || (!mainHandActive && i == playerWpns.indexOfEquippedSpecialWeapon))
                {
                    Color tempColor = weaponButton.mainButton.image.color;
                    tempColor.a = 1f;
                    weaponButton.mainButton.image.color = tempColor;
                }
            }
        }
        ...
        while (displayed < maxDisplayed){
            displayed++;
            Instantiate(this.weaponButton, gridTransform)
                .GetComponent<WeaponButtonUI>()
                .mainButtonForeground.SetActive(false);
        }
    }
